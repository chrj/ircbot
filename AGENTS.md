...
- TLS is tested **without** Docker: `transport.rs` spins up an in-process rustls
  listener on loopback with a self-signed certificate generated by `rcgen` (a
  dev-dependency), so `cargo test --features tls` covers the real handshake, the
  trusted/untrusted split, and the `danger_accept_invalid_certs` escape hatch on
  any machine. Reach for this pattern rather than a container when what you need
  is a server that speaks a few lines of a protocol.
...
- `tokio-rustls` deliberately uses `default-features = false` with the `ring`
  ...
  environment. Don't "tidy" this back to default features.
- The crate never installs rustls' process-global `CryptoProvider`. Choosing one
  belongs to the application; a library that installs it behind the caller's back
  breaks an application that wanted a different provider. The provider is passed
  explicitly to `ClientConfig::builder_with_provider` instead.
